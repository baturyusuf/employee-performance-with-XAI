% Generated from main.md by src/governance/round2_manuscript_v4.py.
...
\Title{Beyond Predictive Accuracy: A Reproducible Leakage- and Governance-Aware XAI Audit Protocol for Ordinal Employee Performance Prediction}
...
\abstract{Employee-performance prediction is commonly reduced to one score even though information availability, ordinal error severity, probability quality, extreme-class behavior, subgroup support, and target construction can change the conclusion. We present a traceable audit protocol for ordinal prediction in two public cross-sectional tables. In the frozen canonical INX benchmark, cumulative-threshold XGBoost led macro-F1 (0.6255), whereas Random Forest led quadratic weighted kappa (QWK; 0.6317) and ordinal mean absolute error (MAE; 0.1583) while its rating-4 recall was 0.0000. Changing hyperparameter selection from macro-F1 to QWK changed 37 of 60 model-fold candidate choices, 6 of 9 metric leaders, and all 9 complete model orderings; nominal XGBoost then led QWK at 0.6418, but its rating-4 recall was only 0.0076. From the Primary Leakage-Aware policy (P3) to the timestamp-unverified Prospective-Plausibility policy (P4), macro-F1/QWK changed by −0.2094/−0.3581 under the fixed schedule and −0.1937/−0.3308 after independent retuning. All six prespecified subgroup attributes are reported; the largest department gaps were 0.2179 for macro-F1 and 0.4388 for QWK, as descriptive diagnostics rather than fairness tests. Across five HRDataset\_v14 repetitions, raw-XGBoost macro-F1/QWK was 0.6531/0.5339 under the retained three-class mapping and 0.5847/0.6288 under a distinct four-class estimand; 11 of 14 canonical 10×5 estimates lay inside the repeated-5×5 ranges. Thus, aggregate ordinal improvements do not guarantee recognition of the extreme class, and conclusions remain conditional on selection objective, information policy, support, target mapping, and validation design. Timestamps, construct validity, redistribution rights, and prospective performance remain unresolved. The contribution is not any single method but their operational integration into a traceable evaluation contract.}
\keyword{human-resource analytics; ordinal classification; explainable artificial intelligence; data leakage; nested cross-validation; SHAP stability; probability calibration; proxy diagnostics; reproducibility}
\externalbibliography{yes}
\begin{document}
\section{Introduction}

Algorithmic analysis of employee records can affect people even when a model is framed as decision support. HR data are typically small, organizationally produced, and entangled with prior managerial decisions. Consequently, an apparently strong classifier may exploit variables recorded during or after an evaluation, reproduce structural context, or report probabilities and explanations that have not been audited for reliability. HR scholarship therefore emphasizes accountability, employee reactions, personal integrity, and the limits of purely technical optimization \cite{tambe2019artificial,leichtdeobald2019challenges,kochling2020discriminated,giermindl2021dark}.

The employee-performance literature has compared conventional classifiers on the 1,200-row INX table and related organizational data \cite{archana2019application,lather2019prediction,li2021employee,patel2022ranker,adeniyi2022comparison,nayem2024unbiased,putri2026klasifikasi}. Such studies establish practical interest, but predictive ranking alone does not answer whether information was available before the rating, whether tuning remained inside training data, whether an ordinal error crossed two rating levels, or whether post-hoc explanations are stable and faithful.

This study contributes:

\begin{itemize}
\item a prespecified P0–P5 information contract distinguishing outcome-proximal, sensitive, timing-uncertain, and organizational-proxy fields;
\item a nine-system ordinal benchmark plus repeated nested-cross-validation and policy-specific retuning, all based on exactly-once held-out predictions;
\item exact-fold TreeSHAP, stability, deletion, calibration, subgroup, and proxy-use diagnostics with explicit noncausal and support-aware boundaries; and
\item an independently trained HRDataset\_v14 mapped-target replication plus a hash-bound claim matrix linking reported numbers to frozen source rows.
\end{itemize}
...
\subsection{Employee-performance prediction and HR decision support}

Four verified studies in the bounded literature set use the exact INX data, while others use psychometric, organization-specific, HRDataset\_v14, or newly collected employee-performance data \cite{archana2019application,lather2019prediction,li2021employee,patel2022ranker,adeniyi2022comparison,nayem2024unbiased,putri2026klasifikasi}. Related churn studies illustrate broader machine-learning and explanation work in people analytics but address different outcomes \cite{abufaty2025integrating,chaudhary2025integrated}. Success on churn cannot be reinterpreted as validation of an ordinal performance-rating model.

\subsection{Ordinal modelling and evaluation}

Ordinal labels require evaluation that respects order without hiding class-specific behavior. Proportional-odds models impose a shared-slope cumulative-link structure \cite{mccullagh1980regression}, while threshold decompositions adapt binary classifiers to ordered outcomes \cite{frank2001simple}. Ordinal assessment can combine nominal discrimination with distance-aware measures \cite{cardoso2011measuring}, including weighted kappa \cite{cohen1968weighted} and ranked probability scoring \cite{epstein1969scoring,gneiting2007strictly}. These aggregate criteria must still be accompanied by per-class results because a favorable distance-weighted score does not guarantee detection of a rare extreme category.

The benchmark includes Random Forest \cite{breiman2001random}, XGBoost \cite{chen2016xgboost}, and LightGBM \cite{ke2017lightgbm} alongside linear and ordinal systems. Algorithm pedigree does not determine which metric a system will lead, so selection objectives and class-specific behavior are treated as empirical parts of the evaluation contract.

\subsection{Leakage, selection, and reproducible evaluation}

Leakage arises when information encodes the target or would not be available at the claimed decision point \cite{kaufman2012leakage,kapoor2023leakage}. Evaluating a configuration on the data used to select it also produces optimistic estimates, motivating nested separation of selection and evaluation \cite{cawley2010overfitting}. Reproducibility guidance calls for transparent processing, sample allocation, hyperparameters, uncertainty, and artifact reporting \cite{mitchell2019model,pineau2021improving}. Our protocol combines these principles through persisted folds, training-only selection, exact output identities, and hashes.

\subsection{Explanation stability and faithfulness}

SHAP provides additive feature attributions, including efficient algorithms for tree models \cite{lundberg2017unified,lundberg2020local}. Yet an attribution plot is not evidence of causality or reliability. Nearby inputs can produce unstable explanations, sensitivity and infidelity can be quantified, and post-hoc explanations can be manipulated \cite{alvarezmelis2018robustness,yeh2019infidelity,slack2020fooling}. We bind each held-out explanation to the exact prediction model, aggregate encoded columns to declared raw-feature families, and examine ranking stability separately from deletion behavior.

\subsection{Calibration and subgroup/proxy boundaries}

Classification and ordinal scores do not show whether probabilities are reliable. Post-hoc calibration can improve some metrics, but calibration is multidimensional and scalar summaries depend on the event and binning scheme \cite{guo2017calibration,vaicenavicius2019evaluating}. In HR settings, removing direct attributes also does not remove every proxy channel or establish fairness. Our subgroup results are support-aware descriptive diagnostics, while department reconstructability is kept separate from performance-model output dependence.

\subsection{Bounded positioning}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{\detokenize{assets/figures/main/figure_01_audit_protocol.png}}
\caption{Audit protocol and evidence-identity flow. Training-only operations remain separated from untouched outer-test prediction, explanation, and evaluation paths.}
\label{fig:1}
\end{figure}

\subsection{Datasets, targets, and data quality}

INX is the primary development and internal out-of-fold (OOF) evaluation table. It contains 1,200 rows and 28 columns; target support is 194/874/132 for ratings 2/3/4. HRDataset\_v14 contains 311 rows and 36 raw columns. The retained three-class mapping combines \texttt{PIP} and \texttt{Needs Improvement} as class 2, preserves \texttt{Fully Meets} as 3, and maps \texttt{Exceeds} as 4, producing support 31/243/37. A prespecified sensitivity retains the two lower source categories as distinct ordered classes, producing four-class support 13/18/243/37. These are different estimands; neither validates equivalence between organizations' rating constructs.
...
\caption{Dataset roles, target mappings, and aggregate audit status}
\label{tab:1}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllll}
\toprule
...
HRDataset\_v14, retained three-class mapping & Independent protocol replication & 311 & 2=31; 3=243; 4=37 & Different features, semantics, parameters, and population \\
HRDataset\_v14, four-class sensitivity & Target-formulation sensitivity & 311 & 13/18/243/37 & Distinct estimand; not an accuracy-improvement comparison \\
\bottomrule
\end{tabular}}
\end{table}

\subsection{Prespecified information policies}
...
\begin{table}[H]
\centering\scriptsize
\caption{P0–P5 information policies}
\label{tab:2}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llll}
\toprule
...
\subsection{Preprocessing, models, and ordinal construction}
...
We report selected-candidate changes, metric-leader changes, complete-ordering changes, and metric-effect magnitudes separately. A lower-rank swap alone is not interpreted as material dependence. Metrics are macro-F1, balanced accuracy, QWK \cite{cohen1968weighted}, ordinal MAE, two-level reversal rate, normalized ranked probability score (RPS) \cite{epstein1969scoring,gneiting2007strictly}, log loss, multiclass Brier score, and top-label expected calibration error (ECE). An empirical-prior predictor, estimated from outer-training labels, is retained as a probability-quality reference; zero top-label ECE for this constant predictor is not evidence of perfect calibration.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{\detokenize{phase5b_figures/figure_02_nine_system_benchmark.png}}
\caption{Nine-system ordinal benchmark. Metric-specific leaders are shown under the common P3 protocol; the plot is not a universal leaderboard.}
\label{fig:2}
\end{figure}
...
\subsection{Fixed-schedule and retuned information-policy contrasts}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{\detokenize{assets/figures/main/figure_03_feature_policy_sensitivity.png}}
\caption{Fixed-schedule feature-policy sensitivity. P0 is diagnostic only; P4/P5 do not establish prospective validity or absence of proxies.}
\label{fig:3}
\end{figure}

\subsection{Exact-fold SHAP stability and deletion diagnostics}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{\detokenize{assets/figures/main/figure_05_global_grouped_shap.png}}
\caption{Global grouped exact-fold TreeSHAP attribution. Magnitudes are raw-margin model attributions and must not be read as causal effects.}
\label{fig:4}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{\detokenize{assets/figures/main/figure_06_shap_stability.png}}
\caption{SHAP ranking stability across folds, model seeds, and outer-training resamples. Pairwise comparisons are dependent and descriptive.}
\label{fig:5}
\end{figure}

\subsection{Cross-fitted calibration}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{\detokenize{assets/figures/main/figure_04_calibration.png}}
\caption{Raw and cross-fitted sigmoid calibration diagnostics. Reliability curves retain bin support and do not provide prospective probability validation.}
\label{fig:6}
\end{figure}

\subsection{Support-aware subgroup and proxy diagnostics}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{\detokenize{assets/figures/main/figure_07_hrdataset_replication.png}}
\caption{HRDataset\_v14 mapped-target replication. Results do not imply target equivalence or locked-model transport.}
\label{fig:7}
\end{figure}

\subsection{Evidence identity and claim control}
...
\subsection{Metric-specific benchmark leaders and extreme-class behavior}
...
\begin{table}[H]
\centering\scriptsize
\caption{Nine-system exactly-once OOF benchmark under P3}
\label{tab:3}
\resizebox{\textwidth}{!}{%
...
\subsection{Selection-objective sensitivity}
...
\begin{table}[H]
\centering\scriptsize
\caption{Selection-objective sensitivity for the four leading trained systems}
\label{tab:4}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllllll}
\toprule
...
\subsection{Repetition variability and ranking}
...
\begin{table}[H]
\centering\scriptsize
\caption{Five-repetition nested-CV summaries}
\label{tab:5}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllll}
\toprule
...
\subsection{P3→P4 timing/information sensitivity}
...
\begin{table}[H]
\centering\scriptsize
\caption{Fixed-schedule and independently retuned policy results}
\label{tab:6}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllllll}
\toprule
...
\subsection{Explanation stability and calibration}
...
\begin{table}[H]
\centering\scriptsize
\caption{SHAP and probability-quality diagnostics}
\label{tab:7}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lll}
\toprule
...
\subsection{Subgroup results across all prespecified attributes}
...
\begin{table}[H]
\centering\scriptsize
\caption{P3 descriptive subgroup gaps at support threshold 30}
\label{tab:8}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllll}
\toprule
...
\begin{table}[H]
\centering\scriptsize
\caption{Proxy-use and reconstructability diagnostics}
\label{tab:9}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lll}
\toprule
...
\subsection{HR target-mapping and CV-design sensitivity}
...
\begin{table}[H]
\centering\scriptsize
\caption{HR target-mapping and CV-design sensitivity}
\label{tab:10}
\resizebox{\textwidth}{!}{%
...
\subsection{HR target-alias and data-quality sensitivity}
...
\begin{table}[H]
\centering\scriptsize
\caption{HR target-alias sensitivity with matched evaluation population}
\label{tab:11}
\resizebox{\textwidth}{!}{%
...
\subsection{Timing and information access define the estimand}
...
\subsection{Subgroup and proxy evidence are descriptive boundaries}
...
\subsection{Target mapping and validation design change the question}
...
\subsection{Explanations and calibration require metric-specific evidence}
...
Eighth, HRDataset\_v14 is independently trained protocol replication, not locked-model transport. Feature space, semantics, parameters, target formulation, and population differ. CV-range inclusion is not equivalence, and alias exclusion does not validate either source field as ground truth.
...
\authorcontributions{[AUTHOR TO COMPLETE: approve CRediT roles for Muhammed Yusuf Batur and Mehmet Göktürk. Do not infer roles from repository activity.]}

\funding{[AUTHOR TO COMPLETE: provide funder and grant number, or explicitly confirm no external funding.]}

\institutionalreview{[AUTHOR/INSTITUTION TO COMPLETE: institution, review unit, determination, reference/application number, date, and approved wording. No approval, exemption, or not-applicable determination is asserted.]}

\informedconsent{[AUTHOR/INSTITUTION TO COMPLETE: provide approved consent-applicability wording tied to provenance and ethics determination. No waiver or not-applicable determination is asserted.]}

\dataavailability{Aggregate evidence, schemas, hashes, and qualified upstream locators are provided in the repository. Raw employee-level datasets are excluded because no dataset has a complete authoritative source-to-byte and redistribution-rights chain. A final immutable release identifier and archive URL remain to be supplied.}

\acknowledgments{[AUTHOR TO COMPLETE, or remove this section.]}

\conflictsofinterest{[AUTHOR TO COMPLETE: provide disclosures approved by all authors, or explicitly confirm none.]}
...
The authoritative bibliography is \texttt{references.bib}; every cited entry belongs to the bounded, source-verified literature set.
