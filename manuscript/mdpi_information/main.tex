% Generated from main.md by src/governance/manuscript_revision_v3.py.
...
INX is the primary development and internal out-of-fold evaluation table. It contains 1,200 rows and 28 columns; target support is 194/874/132 for ratings 2/3/4. HRDataset\_v14 contains 311 rows and 36 raw columns. Its retained mapping combines \texttt{PIP} and \texttt{Needs Improvement} as class 2, preserves \texttt{Fully Meets} as 3, and maps \texttt{Exceeds} as 4, producing support 31/243/37. This study estimand does not validate equivalence between organizations' rating constructs.
...
HRDataset\_v14 & Independently trained mapped-target protocol replication & 311 & 2=31; 3=243; 4=37 & Different features, semantics, parameters, and population \\
\bottomrule
\end{tabular}}
\end{table}
...
\subsection{Nine-system ordinal benchmark}
...
\subsection{Fixed-schedule and retuned policy contrasts}
...
\subsection{Independent mapped-target replication}
...
Each compact package records source run, inputs, exclusions, hashes, and manifest. The frozen Phase 5A matrix links 45 approved claims to source rows. Its SHA-256 is \texttt{1664b188df14135d3b3de8642de3d080c25a3fbadc440615c2bd04b2f14ddabe}; this manuscript is checked only against that boundary (C013). No model, calibrator, SHAP, bootstrap, or paid service call was rerun during assembly.

\section{Results}

\subsection{Metric-specific benchmark leaders}
...
\subsection{Information-policy sensitivity}
...
\subsection{Explanation stability and deletion behavior}
...
\subsection{Subgroup and proxy diagnostics}
...
\subsection{Replication and data-quality findings}

Under the retained HRDataset\_v14 mapping, raw XGBoost mean macro-F1 was 0.6531 across five repetitions (C701). Sigmoid-XGBoost mean QWK was 0.6044 (C702), although its macro-F1 was lower than raw. Mapped class 2 contained 31 records (C703). Repetition ranges are not confidence intervals.

The INX table contained 1200 rows (C801). HRDataset\_v14 contained 215 effective missing cells (C802): 207 termination dates aligned with non-terminated records and eight manager identifiers. Three of 29 declared rules had findings (C803), totaling six rule occurrences (C804): two review-before-hire cases, two target-text/identifier disagreements, and two department-text minority cases. Occurrences are not asserted as six unique erroneous employees; source values were retained.

\begin{table}[H]
\centering\scriptsize
\caption{HRDataset\_v14 replication and data-quality results}
\label{tab:8}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lll}
\toprule
...
\subsection{Accuracy alone obscures the result}
...
\subsection{Information access is part of the estimand}
...
\subsection{Explanations require identity and behavioral checks}
...
\subsection{Calibration and proxy evidence are multidimensional}
...
Eighth, HRDataset\_v14 is independently trained mapped-target replication, not locked-model transport. Feature space, semantics, parameters, and population differ.
...
\institutionalreview{[AUTHOR/INSTITUTION TO COMPLETE: institution, review unit, determination, reference/application number, date, and approved wording. No approval, exemption, or not-applicable determination is asserted.] (C012)}
...
The authoritative bibliography is \texttt{references.bib}; every cited entry belongs to the frozen, source-verified 25-work set.
