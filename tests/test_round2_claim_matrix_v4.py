...
def test_round2_claim_package_is_pending_and_complete() -> None:
    ...
    assert receipt["bibliography_editing_authorized"] is False
    assert receipt["reviewer_response_editing_authorized"] is False
...
def test_every_historical_and_round2_item_has_an_explicit_disposition() -> None:
    ...
    assert all(row["approval_status"] == "pending_user_approval" for row in rows)
...
def test_digest_and_authorization_are_not_inferred() -> None:
    ...
    assert config["status"] == "pending_user_approval"
    assert config["user_approval"]["status"] == "pending"
    assert approval["claim_set_sha256"] == digest
    assert approval["approved_claim_set_sha256"] is None
