...
Requirements 8–10 now have complete Phase 2A implementation and real-data evidence, while their frozen pre-v3 baseline classifications below are retained for audit history. `SHAP_STABILITY_FAITHFULNESS_PROTOCOL.md` formalizes the signed grouping and raw-margin additivity order, adds fixed-fold seed and 80% outer-training resampling designs, and evaluates top-1/3/5 training-reference deletion against 20 random baselines. The 100-fit clean-commit run passed independent source, membership, ranking, perturbation, and summary recomputation. The compact non-row-level package is `phase2a_shap_stability_faithfulness/`; it retains pair dependence and out-of-distribution masking limitations and authorizes no causal or human-usefulness claim.
