...
Status: **scientific implementation and Round 2 rewrite complete; author/release blockers remain open**
...
## Manuscript rewrite completion

The approved boundary has now been applied to `manuscript/mdpi_information/main.md` and the generated `main.tex`. The bibliography contains the 25 previously verified works plus nine verified core ordinal/model references. The main text visibly reports selection-objective sensitivity, extreme-class failure, P3→P4 timing/information sensitivity, all six subgroup attributes, HR target-mapping sensitivity, CV-design sensitivity, and the matched-population target-alias comparison. It states explicitly that aggregate QWK or MAE improvement does not guarantee rating-4 success.

Visible claim codes and the full digest were removed from ordinary manuscript prose. Complete identifiers, selectors, values, qualifiers, and hashes are retained in `SUPPLEMENTARY_EVIDENCE_LEDGER_ROUND2.csv`. The compact `round2_manuscript` package includes deterministic validation, a manuscript claim trace, the reviewer response, final review simulation, author-action list, and original-versus-revised diff. Its status is scientifically revised but not submission-ready or release-authorized.
