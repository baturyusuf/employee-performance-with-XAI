...
- Compact export and validation: exit 0; eight files/24,079 bytes; nine stability, 63 faithfulness, six contrast, and 21 deletion-AUC summary rows; manifest SHA-256 `4cd7bcace03d556e2bd27eea4dca87143745914ddd56136476edc6c662e44481`; no per-sample rows.
- Post-publication Phase 2A focus: 22 passed in 136.65 seconds. Closed-world safety, source equivalence, deterministic re-export, tamper rejection, and byte-preservation regression passed.
- Complete pytest: exit 0; 882 passed, 2 skipped, and 11 subtests passed in 329.68 seconds.
...
- Exact repaired run: exit 0; 2,300 XGBoost fits, 150 baseline fits, 12,440 calibration-training OOF rows, 175 replayable calibrator rows, 15,550 system OOF rows, and zero network/paid-API calls.
- Independent Phase 3A run validator: exit 0 in 12.49 seconds; 18 closed-world files, seven generation implementation blobs, 12 exact sources, ten reconstructed fold systems, 400 selection rows, 150 replayed baselines, 50 refitted calibrators, zero maximum outer sigmoid replay error, and complete result recomputation passed.
- Run-validator focus: 4 passed in 11.09 seconds. Initial combined validator/export focus had 6 passed, 1 failed, and 1 expected absent-package skip in 55.57 seconds; the only failure was an error-message assertion because byte-length tampering was rejected before its hash. The corrected tamper test passed in 22.94 seconds.
- Final pre-checkpoint Phase 3A contract/runner/run-validator/compact-export focus: 23 passed, 1 expected absent-package skip in 59.27 seconds. Two temporary exports were byte-identical; closed-world, row-count, employee/calibration/fold exclusion, boundary-language, and tamper-rejection checks passed. Compileall over `src`, `tests`, and `tools`, `git diff --check`, and manuscript no-change check exited 0.
- Post-publication Phase 3A plus CI-workflow focus: 28 passed in 70.22 seconds. The tracked package, two deterministic temporary exports, complete source recomputation, tamper rejection, and 60-link README audit passed.
- Complete pytest after tracked Phase 3A publication: exit 0; 950 passed, 2 skipped, and 11 subtests passed in 553.20 seconds.
...
## Major-revision v3 provenance/licence and release-readiness validation — 2026-09-07

- Contract-only validation: exit 0; four datasets, ten compact artifact components with 203 files/13,056,035 bytes, eight declaration fields, eleven open blockers, five historical raw-data paths, zero raw redistribution approvals, zero remote tags/releases/DOIs, and zero paid API calls.
- Initial focused regression: 12 tests passed and one assertion failed because the test omitted the article “the” from an exact prohibited-assertion string. The test was corrected without changing the contract; final Phase 4B focus passed 13 tests in 3.60 seconds.
- Related data-acquisition, explicit-input, dataset-card, sanitized-publication, and CI-workflow regression: 34 passed in 6.14 seconds. Compileall, diff hygiene, and manuscript/reference no-change checks passed.
- Implementation checkpoint `1b08313633ba0c3bd343dc5b7175f87029b9425b` pushed normally with local/origin divergence 0/0.
- Clean-commit export and explicit validation: exit 0; 12 files/38,324 bytes, four dataset rows, ten artifact rows, eight declaration rows, eleven blocker rows, contract SHA-256 `f47f402756108377a6eae744bb6eae2684071d80b063571fdd58b0809a372970`, and manifest SHA-256 `d166397d1a0c08df89c893cc092902d2c69671d0c5a52055f3a298bb4fe6ea9e`. Package attribute returned `text: unset`.
...
- Clean approval gate: exit 0 at `5fb1ccb6128f83f6c18d15e61abb4289c5cbbfbc`; 2,061 tracked files, 64 README links, zero raw/environment/large/secret/machine-path findings, and local/origin divergence 0/0.
- Approved package refresh and explicit validation: exit 0; nine files/69,458 bytes, 45 claims, 32 numerical, 13 narrative, 27 sources, 12 components, approval `approved`, manuscript authorization true, unchanged claim-set SHA-256 `1664b188df14135d3b3de8642de3d080c25a3fbadc440615c2bd04b2f14ddabe`, contract SHA-256 `7fc7e66f318613eec6af39b20c494d22b4ead4b746404405e36fdb39689c3403`, manifest SHA-256 `055eb00478acfbb09c8e7bf3147a665a71ce85ceb0275f094a6f09afc7eff7dc`, and zero paid API calls.
- Approved-package staged audit: exit 0; all eight manifest records matched working-tree SHA-256/size and Git-index bytes exactly; nine package files and 14 total checkpoint paths were staged; zero raw/run/model/spreadsheet/manuscript-source paths were included and no unstaged change remained.

## Major-revision v3 Phase 5B manuscript revision — 2026-09-08

- Initial Phase 5B focus: 27 tests passed. Full manuscript-related regression: 78 passed.
- Manuscript/package validation: exit 0; 45 approved claims, exact approved digest `1664b188df14135d3b3de8642de3d080c25a3fbadc440615c2bd04b2f14ddabe`, all 32 numerical claim IDs traced, 25 verified citations used, seven figures, eight tables, exact Markdown/LaTeX figure and table parity, and zero obsolete/prohibited wording findings.
- First complete pytest: one failure, 1,009 passed, two skipped, and 11 subtests passed in 631.13 seconds. The only failure was a secret-scan false positive caused by a hyphenated historical-diff phrase containing the scanner's synthetic key prefix. No credential existed. Diff generation now redacts every matching byte pattern; the corrected security focus passed nine tests.
- Corrected complete pytest before final visual correction: exit 0; 1,011 passed, two skipped, and 11 subtests passed in 607.20 seconds.
- Visual QA inspected the audit-protocol, benchmark, calibration, and replication figures and found one content/caption mismatch in Figure 2. After replacing it with the aggregate-source-generated nine-system/four-metric plot, the relevant manuscript/package suite passed 23 tests and the final builder suite passed eight tests. `git diff --check` passed apart from PowerShell's line-ending warnings.
