...
1. Explicitly stage and checkpoint only the tested Unit 2F config/code/tests/logs. Never stage `reports/manuscript_final/trials/`.
2. Complete the read-only Unit 2G audit, then refactor HRDataset_v14 as the core independent mapped-target replication with current-run/hash/task/claim binding; remove any canonical dependency on legacy fairness helpers.
...
4. Freeze remaining core figure/table, external-replication, lock/CI and side-input contracts before another expensive benchmark execution.
