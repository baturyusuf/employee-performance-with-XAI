...
13. In progress: Phase 3A HRDataset_v14 target-mapping and repeated nested-CV sensitivity contract/runner, fit-free preflight, and bounded diagnostic are complete. Next checkpoint the implementation, execute the exact clean-commit 2,300-XGBoost-fit run, independently validate/recompute it, and publish only compact non-row-level evidence. Then proceed to data quality, literature, provenance, and deliverable phases in `../major_revision_v3/PLAN.md`.
