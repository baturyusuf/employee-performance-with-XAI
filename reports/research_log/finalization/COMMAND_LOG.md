...
Implemented isolated v3 model, evaluation, protocol-validator, and experiment-runner modules plus an ignored local run root. The protocol binds the canonical loader/acquisition files, feature contract, nominal model registry, exact canonical-v2 fold artifacts, and exact v2 nominal OOF predictions. The runner reuses four nominal OOF sets without relabelling, fits only two ordinal models and three naive baselines, verifies exactly-once OOF/probability/fold alignment, generates aggregate/per-class/confusion evidence, and requires clean Git plus stable source/input identities before atomic publication.

Initial ordinal model/evaluation focus passed 19 tests. Protocol validation then passed 32 tests and returned two ordinal models with 6/8 candidates, three baselines, four nominal sources, 16 aggregate metrics, and zero model fits. Runner tests initially passed 38 with six warning-only single-label synthetic cases in 28.65 seconds; the fixtures and fit wrapper were tightened, and the final related regression set passed 65 in 3.80 seconds. Compile and diff checks passed.

Fit-free real-data preflight exited 0 in 3.536 seconds: 1,200 samples, target support 194/874/132, 20 P3 features, 10 outer/5 inner folds, four nominal models/4,800 exactly aligned OOF rows, and all four canonical-v2 source hashes exact. A deliberately incomplete outer-fold-1 diagnostic performed real fits in memory, exited 0 in 9.743 seconds, yielded 14 candidate/five fold/600 OOF rows, and carried `diagnostic_incomplete_never_canonical`; nothing was persisted. No API/network call, manuscript change, or v2 artifact mutation occurred.

The first complete regression run reported 808 passes, two skips, 11 subtests and one stale publication-export assertion after 131.28 seconds. That assertion prohibited any post-generation file under `src/` or `configs/`, including additive v3 files. It was tightened to require every post-v2 scientific-path change to be an added v3-named file, with only the already committed Phase 1A filename exception; modification or deletion of any frozen v2 scientific source remains prohibited. Corrected export/ordinal focus passed 45 in 3.24 seconds. A final complete regression run is required before checkpoint.

Final complete pytest passed 809 tests, two skips, and 11 subtests in 119.88 seconds. No warning or failure remained.

## Major-revision v3 ordinal benchmark execution and compact evidence — 2026-09-03

The complete Phase 1B runner executed from clean pushed commit `dc5cb8b96b096bb2efc6c242403b7e51f870a01b` and atomically published the ignored local run `phase1b_v3_20260903T130912Z_dc5cb8b`. The run produced nine files: 6,000 newly fitted extension OOF rows, 10,800 combined nine-system OOF rows, 140 candidate-search rows, 50 extension-fold metric rows, 50 selection records, 144 aggregate metric rows, 27 per-class rows, and 81 confusion cells. Every system has exactly 1,200 OOF observations on the common persisted folds. Scientific-input SHA-256 is `09e39f0920369ecfbc5be28d731833c7695e2618de47825ffa4529362dfc0b2a`; benchmark-contract SHA-256 is `39bcc62580515888783120a00ed807c0ede0f4c46f587f50897aced4c7999b02`. Network and paid-API call counts are zero.

An independently implemented post-run validator passed. It enforces the nine-file closed world, rehashes all output files, checks exact configuration and principal implementation blobs at the generation commit, rehashes the four immutable canonical-v2 source artifacts, verifies fold/target/evidence-source lineage and probability simplexes, proves byte/numeric reuse of all four nominal OOF sets, and recomputes aggregate/per-class/confusion results from the combined OOF table. It does not claim retrospective recomputation of the generation-time whole-tree hash because legacy line-ending bytes differ between the current worktree and Git blobs; instead it checks internal whole-tree receipt consistency and exact key generation blobs.

The compact exporter first validates that full local run, then atomically publishes only aggregate/fold-level evidence and provenance. Export validation passed with nine files, 71,654 bytes, 144 aggregate rows, 27 per-class rows, 81 confusion rows, 50 extension-fold rows, 50 selection rows, and 14 candidate-summary rows; manifest SHA-256 is `b97b558d8c373958d79847b4e8d568ca6f670e5438859df56213799601f3d389`. Employee-level OOF records, raw data, and fitted models are absent. Re-export comparison was byte-identical, and deliberate README tampering was rejected.

The first staged-byte comparison correctly found that Git's default Windows text normalization changed the bytes of five direct source-result CSV copies, which would have invalidated their manifest hashes after checkout. The compact package received an explicit `-text` byte-preservation rule plus scoped whitespace handling, its index entries were rebuilt without deleting working files, and the repeated eight-record staged-manifest comparison found zero hash mismatches. A dedicated regression now requires the byte-preservation attribute.

Scientific interpretation remains bounded. Cumulative-threshold XGBoost leads macro-F1/balanced accuracy (0.6255/0.6745), Random Forest leads QWK/ordinal MAE (0.6317/0.1583), LightGBM leads RPS/Brier (0.0804/0.3173), and nominal XGBoost leads log loss (0.5515). The proportional-odds model is weaker than the nominal models. Cumulative-threshold XGBoost's log loss is 1.3053, and its macro-F1 advantage over nominal XGBoost is only 0.0044 without an interval or significance claim.

Checkpoint `10d88dc95f322ef620d8f5bbad9a1af8ec73dd77` (`feat(results): publish compact ordinal benchmark evidence`) passed the clean repository gate with 1,878 tracked files, 48 resolved README links, and zero raw/environment/large/secret/machine-path findings. Normal push advanced `dc5cb8b..10d88dc`; local and origin were synchronized 0/0.

## Major-revision v3 repeated nested-CV implementation — 2026-09-04

Work resumed from clean synchronized `10d88dc`. The new contract freezes five repetitions, 5×5 outer/inner folds, five exact outer/inner/model seed triples, six tuned models plus three naive baselines, exact source registries, all 16 per-repetition metrics, four priority variability metrics, descriptive ranges, and six-model ordering stability. The bounded choice is made before result inspection: 5,725 estimator-fit calls rather than 11,450 for ten repetitions. Canonical-v2 OOF reuse and result-selected seeds/repetitions are prohibited.

The runner generates five distinct semantic fold assignments, refits all nine systems, performs training-only candidate selection, enforces exactly-once OOF coverage and probability simplexes, computes repetition variability/rank/winner/Spearman evidence, records exact source/Git/runtime identities, and publishes only from a clean unchanged source tree under the ignored local run root. Process-wide scientific execution denies network and paid-API activity.

Contract validation returned SHA-256 `5681e521cfbaff5963494212fcc047116056fd7d66393346df0463aef7553af9`, 30 nominal and 14 ordinal candidates, 25 outer partitions, 5,725 planned fits, and zero executed fits. Fit-free real-data preflight covered 1,200 rows, target support 194/874/132, 20 P3 features, five valid fold contracts, and five distinct semantic outer-assignment hashes.

One real-data diagnostic for repetition 1/outer fold 1 ran in memory under `diagnostic_incomplete_never_canonical`. It completed 229 estimator-fit calls in approximately 52 seconds and produced 44 candidate rows, nine selected/fold rows, 2,160 held-out prediction rows, and 144 repetition-metric rows; it persisted nothing. The runtime gate recorded zero attempted/successful network operations and zero paid-API calls.

Implementation checkpoint `f71077e98a464ce93351f82dc8d53b7659364096` passed the clean repository gate/preflight and was pushed with local/origin synchronization 0/0. The first full-run attempt `phase1c_v3_20260903T214819Z_f71077e` then failed before output-directory creation or any model fit: the shared digest helper required 64 characters for both SHA-256 values and the 40-character Git commit. The helper now accepts an explicit digest length and the Git gate requires exactly 40 lowercase hexadecimal characters. Under the offline runtime it advances to the intended dirty-worktree rejection, proving the commit-length defect is repaired without weakening cleanliness.

## Major-revision v3 repeated nested-CV execution and compact evidence — 2026-09-04

Checkpoint `78649c426e69fb5270f9d027b11ba6ba87d71a41` was clean and synchronized before execution. Complete run `phase1c_v3_20260903T215015Z_78649c4` finished in approximately 19 minutes 20 seconds and atomically published 12 ignored local files. It produced five distinct fold systems, 1,100 candidate rows, 225 selections, 225 fold metrics, 54,000 OOF rows, 720 repetition metrics, and the prespecified variability/rank/stability summaries. Scientific-input SHA-256 is `b5c5df19ecb1a3d61e01fbfae30f5774b8659d7b038f15ae93adee008130f7da`; runtime network and paid-API counts are zero.

The independent validator rebound all six implementation hashes to exact Git blobs at the generation commit with zero mismatch. It verified the 12-file closed world and output hashes, source contracts, five seed triples, and distinct fold identities; reconstructed each outer assignment and all nested-inner assignments; replayed candidate selection; checked exact OOF coverage, target/fold agreement, probability simplexes, argmax labels, and selected-candidate lineage; and recomputed every fold, repetition, variability, rank, stability, and selected-candidate-frequency result. The validator returned `passed`.

The compact exporter validated the complete run before atomically writing `../major_revision_v3/phase1c_repeated_nested_cv/`. After one provenance-field label was clarified, the exact verified derived package was removed and regenerated from the unchanged source run. The final package has nine files/255,182 bytes and manifest SHA-256 `ce930c634e6108e6a9859df6eaac628bacde98cd7b71563ad0a067dc1849638b`. It contains 720 repetition metrics, 36 variability rows, 120 rank rows, 24 rank-summary rows, four stability rows, 22 tuning-frequency rows, a bounded README, and provenance. Employee-level OOF rows, fold assignments, fold-level metrics, candidate-search rows, raw data, and fitted models are excluded. A package-scoped Git attribute preserves exact manifest bytes on Windows.

Results remain metric-specific. XGBoost has mean macro-F1 0.6288 (SD 0.0099), while LightGBM wins 3/5 repetitions and XGBoost 2/5. Cumulative-threshold XGBoost has mean balanced accuracy 0.6524 (SD 0.0074) and wins 4/5. Random Forest wins QWK and ordinal MAE in all five repetitions with means 0.6311 (SD 0.0022) and 0.1597 (SD 0.0015), respectively. Descriptive ranges are not confidence intervals and no universal winner is admitted.
...
## Major-revision v3 policy-retuning execution and compact evidence — 2026-09-04

Implementation checkpoint `823c84866b461266c75f3224527f679a86ab670e` was pushed and synchronized 0/0 before scientific execution. An initial production invocation without `--run-id` failed at argument validation before fitting or persistence. Complete run `phase1d_v3_20260904T063324Z_823c848` then executed from the clean checkpoint in approximately 10 minutes 44 seconds and atomically published the exact 12-file local package. It produced 480 candidates, 60 selections, 7,200 fixed plus 7,200 retuned OOF rows, 120 fold metrics, 192 aggregate metrics, 96 full comparisons, and six headline rows. Scientific-input SHA-256 is `79bbbca3911c164daa85ab2dde3f8a45a7cf19af83856f104703a01b08e3a811`; runtime network and paid-API counts are zero.

The independent validator rebound five implementation hashes to exact generation Git blobs and every contract/source hash, verified the closed-world output hashes and offline receipt, reconstructed policy features and canonical fold/target lineage, proved exact fixed P0–P3 reuse, replayed all policy/fold candidate selections, checked probability/label/exactly-once OOF invariants, confirmed P4/P5 fixed-schedule lineage, and independently rebuilt every result table. P3 maximum probability replay error is zero.

The compact exporter validated the complete run and atomically wrote `../major_revision_v3/phase1d_policy_retuning/`. The seven-file/92,567-byte package has 192 aggregate rows, 96 comparison rows, six headline rows, 21 candidate-frequency rows, bounded README, provenance, and manifest SHA-256 `57f00a27f7de87f2f931fef1b29f7435e5869f77b14f065d98b7dbd19d2c1303`. It excludes employee OOF, folds, fold metrics, candidate rows, selections, raw data, and models.

Results are deliberately mixed. Macro-F1 raw differences are +0.0029/+0.0117/+0.0185/+0.0000/+0.0157/+0.0087 for P0–P5. P0 balanced accuracy falls 0.0115; P5 QWK and balanced accuracy fall 0.0344 and 0.0071 while ordinal MAE improves 0.0442. P3 is an exact replay. No significance, causal, or universal-policy inference is admitted.

Exact staged-byte review found all six manifest-declared package files identical to their Git index blobs in both SHA-256 and size, confirmed `text: unset` for the package, and found only the seven allowed compact files. The complete local run and all prohibited row/fold/candidate sources remain ignored and unstaged.

## Major-revision v3 SHAP stability/faithfulness implementation — 2026-09-04

Audited the exact canonical-v2 OOF SHAP path, model artifacts, fold rankings, and stability summary against reviewer Requirements 8–10. The existing implementation already uses exact prediction-producing outer models, axis-normalized multiclass SHAP, signed one-hot-to-family sums, and per-sample/class grouping preservation, but only the 45 dependent outer-fold pairs were reported and no model-level deletion test existed.

Added a hash-bound Phase 2A contract, fail-closed validator, and runner. The design holds exact folds and candidates fixed across five new model seeds, separately performs five deterministic stratified 80% subsamples inside each outer-training fold, explicitly sets SHAP 0.51.0 TreeExplainer to tree-path-dependent raw-margin output with no background, and evaluates top-5/10/15 Jaccard plus all-feature Spearman. Faithfulness uses exact canonical models, top-1/3/5 predicted-class grouped SHAP, outer-training median/mode masks, and 20 random baselines with probability/margin drop and normalized deletion AUC.

Fit-free preflight validated 1,200 samples, 20 features, ten model artifacts, eight candidates, and 100 planned new fits. Exact-model reference explanation produced 1,200×3×20 grouped values, maximum additivity error `5.74e-6`, zero group-sum error, canonical-importance maximum difference `9.1e-17`, and identical ranks. Full ten-fold seed-1044 and resample-11042 diagnostics used 1,080 and 864 training rows per fold and stayed below the additivity tolerance with zero group-sum error.

The first deletion diagnostic exposed a pandas 3 integer-column assignment rejection when a training median was fractional. Numeric mask columns are now explicitly promoted to floating point before inserting the training-only reference. The corrected two-random-baseline diagnostic produced 3,600 guided and 7,200 random rows, all summaries/contrasts/AUC grids, and positive guided-minus-random probability-drop contrasts at top 1/3/5. These diagnostic contrasts are feasibility observations only and are not retained as scientific evidence.

Implementation checkpoint `f6d9ad82e4f51df56268f39a90b3a1de434e65d0` was pushed and synchronized 0/0. The first complete execution attempt `phase2a_v3_20260904T072729Z_f6d9ad8` completed its computational grid but failed closed before atomic publication during the final source rehash: model-index paths are benchmark-relative, while the new redundant rehash initially resolved them from the repository root. The staging directory was removed and no result package was admitted. The path is now resolved explicitly against the validated benchmark directory, with a regression over all ten model files.

## Major-revision v3 SHAP stability/faithfulness execution and independent validation — 2026-09-04

Path-fix checkpoint `6e52de76f7e486985cbc2b32a53b2554c1c6f6c1` was clean and synchronized 0/0 before execution. Complete run `phase2a_v3_20260904T073008Z_6e52de7` finished successfully and atomically published the 14-file ignored local package. It contains 2,200 fold-feature importance rows, 110 additivity checks, 220 run rankings, 75 pairwise stability rows, nine stability summaries, 75,600 sample perturbation rows, 63 faithfulness summaries, six guided-versus-random contrasts, 25,200 sample deletion-AUC rows, 21 AUC summaries, and one AUC contrast. The run performed the prespecified 100 new estimator fits; network and paid-API counts are zero.

An independent closed-world validator now rebinds all output hashes, the contract and source hashes, and five implementation hashes to exact Git blobs at the generation commit. It reconstructs every full/resampled training membership and canonical candidate assignment; recomputes weighted rankings, all Jaccard/Spearman pairs, and stability summaries; checks TreeSHAP output-space/grouping/additivity receipts; replays all 20 deterministic random feature orders; verifies canonical OOF baselines and nested deletion sets; and rebuilds every faithfulness, contrast, feature-frequency, and deletion-AUC output from the 75,600 source rows. The validator returned `passed` with maximum additivity error `6.5115e-6` and zero group-sum error.

Descriptive results are bounded. Mean top-5 Jaccard is 1.0000 for canonical fold, model-seed, and 80% resampling comparisons; mean all-feature Spearman is 0.9066, 0.9945, and 0.9847 respectively. SHAP-guided minus random-repetition-mean probability drops are +0.2676, +0.2481, and +0.2063 after deleting 1, 3, and 5 features; deletion-AUC difference is +0.2208. Pair values are dependent, masking may create out-of-distribution hybrids, and no confidence-interval, causal, fairness, human-usefulness, prospective-validity, or deployment claim is admitted.

Validation/export checkpoint `cd0918ec332a575fd4737a2f80ebfd58ffea1f27` was pushed and synchronized 0/0. The governed exporter then independently revalidated the complete run and atomically wrote `../major_revision_v3/phase2a_shap_stability_faithfulness/`. The eight-file/24,079-byte package has nine stability rows, 63 faithfulness summaries, six contrasts, 21 deletion-AUC summaries, the exact aggregation receipt, bounded README, provenance, and manifest SHA-256 `4cd7bcace03d556e2bd27eea4dca87143745914ddd56136476edc6c662e44481`. All per-sample, fold, membership, pairwise, raw, and model artifacts are excluded.

Exact staged-byte review found all seven manifest-declared package files identical to both their Git-index blobs and working-tree bytes, with zero SHA-256 or size mismatch. The package attribute returns `text: unset`; no sample perturbation, sample AUC, fold importance, pairwise, additivity, model, or serialized-estimator artifact is staged.

## Major-revision v3 calibration diagnostics implementation — 2026-09-04

Audited the canonical calibration protocol, metadata, validation receipt, 2,400 raw/sigmoid OOF probability rows, 60 classwise reliability-bin rows, method summary, intervals, and paired differences. The existing sigmoid method is three one-vs-rest Platt fits on five-fold cross-fitted outer-training probabilities followed by row normalization; it was predeclared and did not use outer-test rows for fitting or selection.

Added `configs/calibration_diagnostics_v3.json`, a fail-closed exact-source validator, and a fit-free atomic runner. The contract freezes ten equal-width bins and boundary/empty-bin handling; separate top-label, one-vs-rest classwise, and cumulative-threshold ECE; normalized RPS and cumulative Brier; deterministic descriptive pooled-OOF calibration intercept/slope; and mixed-result/prohibited-claim controls. The runner publishes only aggregate tables, reliability figures, and receipts under the ignored local run root.

Contract validation passed with 1,200 samples per method, exact raw/sigmoid sample/fold/target alignment, valid probability simplexes, the full legacy reliability grid, and zero planned performance-model or probability-calibrator fits. Fit-free preflight reported 2,400 source rows, ten diagnostic regressions, and zero network/API calls. An in-memory calculation replayed the legacy log-loss, multiclass-Brier, top-label-ECE, and classwise-bin evidence exactly; no scientific package was persisted.

The implementation retains the mixed canonical finding: sigmoid-minus-raw changes are -0.095817 for log loss, -0.079202 for multiclass Brier, +0.004391 for top-label ECE, -0.082040 for macro classwise ECE, -0.059521 for mean cumulative ECE, and -0.019029 for normalized RPS, where lower is better. New diagnostic changes have no inferential interval. Exact-commit execution remains pending until the tested implementation checkpoint is pushed and the worktree is clean.

## Major-revision v3 calibration execution and independent validation — 2026-09-04

Implementation checkpoint `21d1aecb6e61511e95aee498ab81c54fe6e5a6ab` was pushed and synchronized 0/0. The clean post-commit repository gate passed with 1,938 tracked files, 55 README links, and zero raw/environment/large/secret/machine-path findings.

Complete run `phase2b_v3_20260904T120838Z_21d1aec` executed from that clean commit and atomically published its 11-file ignored local package. It contains two metric-summary rows, six classwise diagnostics, four cumulative diagnostics, 120 reliability-bin rows, six method contrasts, two PNG/SVG figure pairs, and receipts. It performed zero new performance-model or probability-calibrator fits and zero network/paid-API calls. Scientific-input SHA-256 is `97797e436e816623d8652772a5089d226f304b59a30ffcdee110c33e413eec86`.

The independent validator binds the exact generation Git blobs and nine canonical source hashes, verifies the output closed world and hashes, independently reconstructs all top-label/classwise/cumulative bins and ECE values, multiclass and binary scores, ten Newton–Raphson calibration regressions, normalized RPS identity, paired legacy intervals, and all six contrasts. It also checks figure dimensions/labels and deterministic SVG metadata. Validation passed with 24 explicit empty bins and maximum regression-information condition number 68.60.

Both figures passed visual inspection: labels, ideal diagonals, raw/sigmoid series, panel ranges, and legibility were intact. Two temporary compact exports were byte-identical and deliberately tampered output was rejected. The exporter excludes employee-level probabilities, folds, raw data, fitted models/calibrators, and method-selection or deployment claims.

## Major-revision v3 calibration compact evidence — 2026-09-04

Validation/export checkpoint `25953b4` was pushed. The governed exporter independently revalidated the complete run and atomically wrote `../major_revision_v3/phase2b_calibration_diagnostics/`. The package has 13 files and 757,267 bytes: five aggregate CSV tables with 2/6/4/120/6 rows, two PNG/SVG reliability-figure pairs, a diagnostic receipt, bounded README, provenance receipt, and manifest SHA-256 `196d9eea87a160b375ddf215fc131142598b0338fa373a7474aaaa1b865d29f3`.

The compact closed world copies every included scientific artifact byte-for-byte from the independently validated run. It excludes calibration-prediction rows, sample/fold identities, stage metadata, raw data, fitted performance models, calibrator objects/parameters, and any method-selection, future-calibration, all-metrics-improved, or deployment claim. A package-specific `-text` attribute preserves its manifest-bound bytes on all platforms.

## Major-revision v3 subgroup/proxy-use implementation — 2026-09-04

Audited the immutable canonical subgroup/proxy package, all 3,600 exact raw OOF performance rows, six department-reconstruction metric intervals, three paired reconstruction differences, subgroup bootstrap metadata, shared 10×5 folds, benchmark lineage, and the ten persisted P3 XGBoost models. Exact P3 subgroup-stage probabilities match the benchmark XGBoost OOF probabilities byte-for-value.

Added `configs/subgroup_proxy_use_v3.json`, a fail-closed source validator, an offline atomic runner, focused tests, and `SUBGROUP_PROXY_USE_PROTOCOL.md`. The runner calculates the full 2,025-row supported-group metric grid, 486-row threshold/gap grid, and a 162-cell P3 family with 5,000 pointwise and simultaneous exploratory bootstrap intervals. It separately calculates paired P3/P3-minus-JobRole changes overall and by department, 20 marginal and 20 department-conditional JobRole perturbations using exact persisted outer models, and carries forward reconstruction aggregates with non-use claim boundaries.

Contract validation and fit-free preflight passed with contract SHA-256 `65bf6d1cdf99cda5f764b5ed93aa6b4bf214a5944ca8bf1248d0c804a73c9696`, 1,200 samples per system, ten persisted outer models, 5,000 planned bootstrap draws, 40 planned perturbation repetitions, and zero model fits/network/API calls. A non-persisted in-memory real-data evaluation completed in approximately 66 seconds. All 5,000 draws were complete for the simultaneous family; its deterministic resample hash is `d16aadb56f2dde124df62387447bb01ddf51236f7f9e9e01b6218e8f6265f646`.

The in-memory paired comparator has mean total-variation change 0.0906, prediction-change rate 0.1075, and macro-F1 change -0.0330 for P3-minus-JobRole minus P3. Mean marginal-permutation total variation/prediction-change/macro-F1 change are 0.1024/0.1305/-0.0432; department-conditional values are 0.0529/0.0739/-0.0034. These diagnostic values are not yet admitted scientific evidence; exact clean-commit execution and independent validation remain required.

## Major-revision v3 subgroup/proxy-use execution and validation — 2026-09-07

Implementation checkpoint `e314bb5d1935da6a23e26b2085330dacb468fe32` passed the clean repository gate with 1,960 tracked files, 57 README links, and zero raw/environment/large/secret/machine-path findings. The branch was pushed normally and synchronized 0/0 with origin.

Complete run `phase2c_v3_20260907T070905Z_e314bb5` then executed from that clean commit and atomically published its 12-file/8,428,003-byte ignored local package. Scientific-input SHA-256 is `2a42827646702378fb92cf0e0b940fa2739334cc6b62fa1f2ff4f9af66787f1e`. It contains 2,025 subgroup metric rows, 486 gap rows, 162 P3 interval rows, 1,200 exact paired-change rows, seven aggregate department rows, 48,000 perturbation rows, 40 repetition rows, two scheme summaries, six reconstruction metrics, three reconstruction differences, and receipts. It made zero new model fits, network calls, or paid API calls.

The independent validator imports no calculation from the Phase 2C runner. It bound all 12 output hashes, 22 immutable source hashes, the exact generation Git blobs, offline receipt, canonical data/schema, OOF system alignment, P3 feature order, and persisted model set. It independently recomputed the complete metric/support/gap grids, replayed all 5,000 fold/class-stratified bootstrap samples and their standardized maximum-deviation family, reconstructed every paired output change, reran all 40 seeded perturbations through the ten exact outer models, rebuilt both summary levels, and verified reconstruction-source byte values. It returned `passed`; all 5,000 draws were complete and the familywise critical value was 5.073969.

The first compact-export test attempt rejected intended department-level mean class-probability-change columns because a broad `prob_class_` token rule could not distinguish aggregate from employee-level fields. The package was only temporary. The guard now rejects employee identifiers, targets, predictions, folds, and sample keys while the closed-world inventory separately excludes both employee-level source files. Corrected temporary exports are deterministic, row-bounded, and pass tamper rejection; tracked publication remains next.

## Major-revision v3 subgroup/proxy-use compact evidence — 2026-09-07

Validator/export checkpoint `33fe6b8430efb83ed9119f047d35f5f43fc41685` passed the clean repository gate with 1,964 tracked files and 57 resolved README links, then pushed normally with local/origin divergence 0/0. The governed exporter independently revalidated the complete local run before and after atomic export.

The resulting `phase2c_subgroup_proxy_use/` package has 12 files/994,542 bytes and manifest SHA-256 `0720d0575154b850a0d3cb89b9bb09403dd514f1e9ebde87ff458189185077f0`. It contains the complete 2,025 group-metric, 486 gap, 162 P3 interval, seven overall/department paired-change, 40 perturbation-repetition, two scheme-summary, six reconstruction-metric, and three reconstruction-difference rows. The diagnostic receipt is a byte copy; bounded README/provenance/manifest files state the support, selection, refit, OOD, reconstructability/use, fairness, causality, and deployment limits.

Closed-world validation confirms that `proxy_prediction_change_sample.csv`, `jobrole_permutation_sample.csv`, stage metadata, folds, raw data, and fitted models are absent. All included source artifacts are byte-identical to the independently validated run. A package-level `-text` attribute was added to preserve manifest-bound bytes on Windows.

## Major-revision v3 HRDataset sensitivity implementation — 2026-09-07

Audited the exact user brief sections 15–19, canonical-v2 HRDataset stage, raw/schema identities, target support, seven-feature primary policy, 10×5 OOF probabilities, calibration implementation, model grid, baseline models, and ordinal evaluation utilities.

Added `configs/hrdataset_sensitivity_v3.json`, `HRDATASET_SENSITIVITY_PROTOCOL.md`, the fail-closed source/decision validator, offline atomic runner, and focused tests. The contract binds 12 immutable inputs, primary support 31/243/37, alternative raw-order support 13/18/243/37, five fixed 5×5 repetition seeds, eight XGBoost candidates, predeclared training-only sigmoid calibration, three naive baselines, ten metrics, and per-class/confusion outputs.

Fit-free preflight exited 0: 311 samples, seven features, ten distinct fold-contract and semantic-assignment hashes, 2,300 planned XGBoost fits, 150 planned baseline fits, and zero actual fits/network/API calls. The first bounded diagnostic failed only when label-only baselines received a scientific feature frame containing missing tenure values; no output was persisted. The baseline path was corrected to use a finite inert design while retaining training-label-only behavior. The corrected two-formulation/one-fold diagnostic exited 0 after 92 XGBoost and six baseline fits, with 630 OOF rows and zero network/API activity.

Implementation checkpoint `656b1e393d3e2e41cd4e455c4b66f71c352c374b` passed the 1,982-file/59-link repository gate and pushed normally with local/origin divergence 0/0. Run `phase3a_v3_20260907T135159Z_656b1e3` then completed 2,300 XGBoost and 150 baseline fits, produced 15,550 OOF rows, and recorded zero network/API activity. During independent-validator design, the local closed world was found not to contain the selected-candidate inner-OOF probabilities needed to reconstruct and replay each sigmoid calibrator. The run is therefore retained locally as superseded/non-admitted evidence despite successful computation.

The runner now persists `calibration_training_oof.csv` locally and emits the full replayable sigmoid settings/parameter/hash schema. These rows and parameters remain prohibited from the future Git compact package. A corrected diagnostic completed and the calibrator-row reconstruction test proves exact replay. The repair passed 100 related tests, the complete 942-test pytest suite, unittest discovery, compileall, diff hygiene, and manuscript no-change checks; the clean repair checkpoint and exact rerun remain next.

Replay-evidence repair checkpoint `f6e6a0af7f36d7d69f028df07b6c0842c25ebc56` passed the clean repository gate with 1,982 tracked files and 59 resolved README links, then pushed normally with local/origin divergence 0/0. Fit-free preflight again passed ten distinct folds, 2,300 planned XGBoost fits, 150 planned baseline fits, and zero actual fits/network/API calls.

Exact run `phase3a_v3_20260907T141213Z_f6e6a0a` completed and atomically published its 18-file ignored local package. Scientific-input SHA-256 is `76518cc8387a1f632b388e2e37f906879dcd73f32c6bc6a13443f15449e896bb`; it contains 400 candidate rows, 50 selections, 12,440 calibration-training OOF rows, 175 calibrator rows, 15,550 system OOF rows, 250 fold metrics, and complete aggregate/class/confusion/design evidence after 2,300 XGBoost and 150 baseline fits. Runtime network and paid-API counts are zero.

The independent validator imports no Phase 3A runner calculations. It bound seven generation implementation blobs and all 12 exact sources, reconstructed ten target-specific fold systems, independently applied the macro-F1/QWK candidate rule, replayed all 150 label-only baselines, reconstructed and refit all 50 sigmoid calibrators, reproduced every outer sigmoid probability with maximum error zero, and recomputed every scientific table. Temporary compact export produced a deterministic 14-file/409,991-byte package containing only aggregate/repetition/class/design evidence. The first tamper-test assertion expected only a hash message while the validator correctly rejected the changed byte length first; the assertion now accepts either size or hash rejection.
...
## Major-revision v3 literature/novelty implementation and compact evidence — 2026-09-07

Verified the selected bibliography against publisher/proceedings pages, authoritative repositories, and the public Crossref REST endpoint; no paid API was called. The resulting contract contains 25 works, six reasoned exclusions, complete source URLs and evidence notes, eleven operational positioning fields, and a bounded mechanism-based novelty statement. Four studies use the exact INX data (one as a secondary dataset), one uses exact HRDataset_v14, 16 DOIs resolve to exact Crossref works, eight verified records have no assigned DOI, and one publisher-asserted DOI remains unresolved.

Added `configs/literature_positioning_v3.json`, the standard-library validator/exporter, and focused tests. The contract-only validator passed; the first temporary export exposed one canonical wording mismatch between generated protocol and validation. The generated sentence was corrected to the exact unresolved-Crossref boundary, after which all eight tests passed. Implementation checkpoint `e408208bbd7fb3071c47717f21ef521b1917e93a` was pushed normally with local/origin divergence 0/0.

From that clean commit, the exporter atomically wrote `phase4a_literature/`. Explicit validation passed its closed-world inventory, exact contract-derived CSV rows, documentation boundaries, provenance receipt, and manifest hashes: nine files/58,534 bytes, 25 positioning rows, six exclusion rows, manifest SHA-256 `e7b8fdd654c228344d13ec37c8817414b977a5173e5ca3721cbd2641b61c1730`, and zero paid API calls. The manuscript and `references.bib` remain unchanged.

Two follow-up command invocations had scope/syntax errors and changed no evidence: one passed a package path directly after the unsupported `--validate-package` spelling, and one referenced a nonexistent `tests/test_ci_workflow.py` while invoking the clean-worktree gate before publication staging was committed. The corrected commands use `--validate-package-only --output-dir ...`, `tests/test_ci_workflow_contract.py`, and defer the repository clean gate until after the publication commit.

## Major-revision v3 provenance/licence and release-readiness audit — 2026-09-07

Queried only public, no-cost publisher/platform metadata and official guidance. The official IABAC brief and dataset endpoint were online; the brief identifies the synthetic project scenario, dataset filename/link, and restrictive reproduction notice, while the endpoint's HEAD metadata reports 410,624 bytes matching the local workbook size without downloading or claiming exact upstream byte identity. The HRDataset_v14 author record identifies the two authors, fictitious teaching dataset, CC BY-NC-ND 4.0 sharing terms, and codebook. The IBM Kaggle record identifies fictional IBM-created data and an Open Database/Database Contents licence label. The historical Employee Turnover Kaggle record returned 404/403; SAS documentation corroborates only that the data are simulated, and no conflicting mirror licence was accepted.

GitHub tree/commit metadata verified that the local HRDataset_v14, IBM, and Employee Turnover files exactly match immutable mirror blobs `d562e61d...`, `b8531237...`, and `569b02db...`; all three mirror repositories have no detected licence, so this proves only byte identity. Repository audit found no current tracked raw dataset, five raw dataset paths in history, no repository software licence, no GitHub release, zero remote tags, and one stale local-only tag at `a47dcdf...`. Ten tracked compact evidence packages contain 203 files/13,056,035 bytes and each manifest hash was frozen.

Added `configs/release_readiness_v3.json`, the offline fail-closed validator/exporter, and 13 focused tests. The first test run had one assertion typo around the article “the” in a prohibited phrase; the test was corrected without changing contract evidence. The final focused suite passed 13/13, the related data-acquisition/publication/CI suite passed 34/34, and implementation checkpoint `1b08313633ba0c3bd343dc5b7175f87029b9425b` pushed with divergence 0/0.

From that clean commit, the exporter atomically wrote and explicitly revalidated `phase4b_release_readiness/`: 12 files/38,324 bytes, four dataset rows, ten artifact rows, eight declaration rows, eleven open blocker rows, and manifest SHA-256 `d166397d1a0c08df89c893cc092902d2c69671d0c5a52055f3a298bb4fe6ea9e`. Package generation made zero network/paid-API calls and created no raw data, tag, release, DOI, licence, ethics determination, history rewrite, or manuscript edit.

## Major-revision v3 claim-matrix implementation — 2026-09-08

Audited the ten published v3 evidence packages, the feature-availability contract, the historical v2 handoff ledger, and the unresolved Phase 4B boundaries. Added `configs/claim_matrix_v3.json`, the offline fail-closed validator/exporter, and focused tests. The contract contains 45 pending-review sentences across 12 components: 32 numerical claims each select one exact SHA-bound CSV row/value and use deterministic half-even display rounding; 13 narrative claims bind exact text anchors and carry support, qualification, and overclaim fields. No manuscript, bibliography, raw-data, release, tag, DOI, ethics, licence, or history state was changed.

The first focused run passed ten tests and failed one assertion because rehashed tampering was rejected at the earlier manifest-content gate while the test expected only the later derived-file gate. The assertion was widened to accept either valid rejection stage without changing evidence or validation behavior. The corrected focused suite passed 11 tests; the related claim/release/literature/CI suite passed 36 tests. Complete pytest passed 1,001 tests, two skips, and 11 subtests; unittest discovery passed 179 tests with one skip. Compileall, diff hygiene, and manuscript/reference no-change checks passed.

Implementation checkpoint `fcfc4692466d7bc9f414db4b2fe693073d0d7f4e` passed the clean repository gate with 2,052 tracked files and 63 resolved README links, then pushed normally with local/origin divergence 0/0. From that clean commit, the exporter atomically wrote and explicitly revalidated `phase5a_claim_matrix/`: nine files/70,041 bytes, 45 claims, 32 numerical claims, 13 narrative claims, 27 source files, all 12 components, claim-set SHA-256 `1664b188df14135d3b3de8642de3d080c25a3fbadc440615c2bd04b2f14ddabe`, contract SHA-256 `a3c55ac3d089b5e38b029d2364edc9d09b29b3ebc8decc3e04b4bce8f2e2c7f6`, and manifest SHA-256 `76be9a39b48fc2577487fc6d90129119642de0ab92afd71ff39cc1aea1f6977d`. The approval record remains pending and package generation made zero network/paid-API calls or manuscript/release mutations.

## Major-revision v3 explicit claim approval — 2026-09-08

Matched the user's explicit authorization to claim-set SHA-256 `1664b188df14135d3b3de8642de3d080c25a3fbadc440615c2bd04b2f14ddabe`. Updated only the approval/provenance state and manuscript/bibliography drafting authorization; the 45 frozen claim objects and their digest remain byte-semantically unchanged. Extended the validator/exporter to validate either pending or approved states, require exact digest matching, render the explicit decision record, and support an atomic validated replacement of the pending review package. Release, tag, DOI, raw publication, history rewrite, and unresolved licence/ethics/declaration states remain fail-closed.

Approval checkpoint `5fb1ccb6128f83f6c18d15e61abb4289c5cbbfbc` passed the clean repository gate with 2,061 tracked files and 64 README links, then pushed normally with divergence 0/0. The exporter validated a staging package before atomically replacing the pending review directory and revalidated the final path. The approved package has nine files/69,458 bytes, manifest SHA-256 `055eb00478acfbb09c8e7bf3147a665a71ce85ceb0275f094a6f09afc7eff7dc`, contract SHA-256 `7fc7e66f318613eec6af39b20c494d22b4ead4b746404405e36fdb39689c3403`, approval state `approved`, and the unchanged approved claim-set SHA-256 `1664b188df14135d3b3de8642de3d080c25a3fbadc440615c2bd04b2f14ddabe`.

## Major-revision v3 Phase 5B manuscript revision — 2026-09-08

Rewrote the Markdown manuscript around the approved ordinal-performance audit contribution, generated the LaTeX peer and a 25-record verified bibliography, refreshed manuscript-support registers, and added deterministic Phase 5B build/validation code plus regression coverage. The builder requires the exact approved digest, validates all 45 approved claims and 32 numerical IDs, rejects obsolete/prohibited wording, enforces Markdown/LaTeX figure/table parity, emits eight table groups and seven figure groups, and creates the bounded 63-file package. It performs no network or paid-API call and publishes no employee-level row.

Committed and pushed the core Phase 5B checkpoint as `bb54e6bda21f11309435d6c8029549247b9b4f2f` (`docs(research): revise manuscript under approved phase 5b boundary`). Visual inspection of the rendered figures then identified a semantic mismatch in Figure 2: a historical four-model plot had been paired with a nine-system caption. Replaced it with a deterministic four-panel plot of all nine Phase 1B systems across macro-F1, balanced accuracy, QWK, and ordinal MAE; updated manuscript paths/manifests/diff; and added an explicit test for its system count and asset identity.

Checked for local LaTeX/Pandoc/Tectonic executables; none was available. The reproducibility record therefore states structural-source validation rather than PDF compilation. Normalized generator-created SVG line endings/line tails and rebuilt the package. Final inventory is 63 files/3,044,248 bytes with 62 manifest records and manifest SHA-256 `4abcf95137a3af0a02aa4e85b111575afaa69237b0af88d62a50de43c164660c`.
