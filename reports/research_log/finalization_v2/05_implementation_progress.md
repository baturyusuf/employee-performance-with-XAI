...
## V2-020 complete-build offline runtime - 2026-07-14

`offline_runtime.py` now supplies the mandatory process-wide boundary used by public `build`. It blocks socket/DNS/connect/send/listener operations, poisons caught attempts, clears/restores API credential variables and limits child processes to exact local read-only Git provenance. The final package-status semantic contract carries zero network/API calls and the exact policy payload; tampering or writing outside an active boundary fails.

Final focused tests pass 107. The first complete run exposed one introspection test tied to the old public function body; after pointing it to `_build_impl`, final pytest passes 725 with two skips/11 subtests, unittest passes 176 with one skip, and compileall passes. A real release-blocked invocation restored credentials and wrote no output. V2-020 is locally complete; V2-019 hosted assertion and the final real package policy remain open.

## V2-019 CI and validation-only release workflow - 2026-07-14

The hosted workflow now runs on push/pull request/manual dispatch with minimal read-only permissions, credential-free checkout, CPython 3.14, the exact development lock, dependency and environment checks, manuscript-file diff rejection, the global offline-runtime focus, complete pytest/unittest/compileall, and the production repository hygiene gate. The gate checks clean Git state, tracked raw/environment/oversize/secret/machine-path absence, the complete unique issue register and local README links.

The manual release workflow is deliberately not a publisher. It targets only a labelled trusted Windows self-hosted runner, requires an exact immutable run ID and full commit, preserves ignored local scientific inputs, and invokes read-only two-scope validation inside the offline boundary. A new public validator shares every strict package check with pointer promotion but never creates or changes `latest`. The workflow has no upload, promotion, Git push, release, Zenodo or scientific-build step.

The locked CPython 3.14 environment exposed a real parser defect: the shared loader preferred PyYAML even though repository configs are documented as JSON-compatible, causing valid JSON exponent numbers to become strings. JSON-first/YAML-fallback loading now gives identical types with or without PyYAML, and a regression freezes that behavior. Final focus passes 35; all 122 failure-affected tests pass; complete pytest passes 730 with two skips/11 subtests, unittest passes 177 with one skip, and compileall passes. Implementation is locally complete; first hosted execution remains required before V2-019 is resolved.

The first hosted run reached full pytest after all earlier gates passed, then correctly exposed that 22 integration tests consume raw inputs intentionally removed from Git. They now declare exact absent-file skips only on data-free hosts. The manual trusted release workflow has the complementary fail-closed rule: all five local inputs must exist before its full test/validation sequence, so no release candidate can use those skips. Local real-data behavior remains 730 passed/two pre-existing skips. Hosted run `29351557672` at follow-up `700adb2` passed all steps with 709 passed/23 explicit skips/11 subtests, unittest 177/10 skips, compileall and the repository gate. V2-019 is resolved.

## V2-021 source-bound production core figures - 2026-07-14

`manuscript_core_figures.py` now implements the frozen seven-figure plan without importing or discovering any historical output. It accepts only the exact active run root and 24 declared sources. Each tabular/JSON source must carry the active run/config/scientific-input identity; the orchestrator separately requires every upstream closed-world stage receipt and source-tree identity.

The runner publishes through a contained sibling staging directory and refuses overwrite or orphan reuse. Its closed output contains exactly 29 runner-owned files: seven 300-DPI PNGs, seven SVGs, seven machine-readable source CSVs, seven technical captions and `figure_manifest.json`. SVG declarations prohibited by the package validator are removed by the writer. Source tables preserve the plotted denominators/intervals and exact source paths while adding run/config/scientific-input/source-tree identity. Captions carry the frozen claim boundary and exact identity. Failed staging is preserved for forensic recovery.

The builder registers `core_figures` as atomic, verifies every source stage, adds the generic closed-world `stage_contract.json`, and runs `validate_core_figure_package` for completion and future cache admission. A realistic synthetic fixture exercises the actual Matplotlib renderer and then passes that production validator. It is test evidence only and is removed by pytest; no real figure artifact, numerical claim, canonical package or manuscript file was created.

Final validation is 733 pytest passes with two skips/11 subtests, 177 unittest tests with one skip, and a clean compileall. Current config hash is `a866bd6f9851fffce9af4238459d05fc1c6540f9e11d3668698a11c23811107d`; current source-tree hash is `fcae16c902fc759be087a9c42c451799bdc97fc49b6c396eb54260086a1e5987`. V2-021 figure generation is implementation-complete; core tables and real canonical execution remain pending.

Checkpoint `5a0d92680fffd0d9f9312a6e22f5274459708a23` was pushed normally. Hosted Actions run `29353799148` completed successfully at that exact SHA.

## V2-022 authoritative metric registry and source tables - 2026-07-14

`task_schema.py` now owns the only metric definition registry. Its 18 entries declare display name, estimand, family, direction, mathematical domain, unit, probability requirement, complete-OOF aggregation, denominator and paired sample-level uncertainty. Five explicit task schemas add applicability and selection roles. The canonical config must exactly partition every metric into applicable/N/A for every task, while `oof_bootstrap.py` reuses the same immutable definitions instead of maintaining a second direction/domain map.

`table_contract.py` freezes the exact non-manuscript source-table plan and claim boundaries: core tables 1-10 plus 13, and supplementary tables 11-13. `manuscript_tables.py` reads only those active-run sources, rejects mixed run/config/scientific/source identities, records source SHA-256/size/path per emitted row, adds dataset/model/evaluation/denominator/uncertainty and metric-schema identity, and atomically publishes a closed-world scope package. A specialized validator runs during builder completion and future cache admission. Failed staging is preserved; historical/latest discovery, manual values and fixture retention are absent.

The core plan covers dataset roles/mappings/support, exact primary policy, four-model benchmark, leakage-policy sensitivity, sigmoid calibration, global grouped OOF SHAP, SHAP stability, subgroup diagnostics, department proxy diagnostics, HRDataset_v14 replication, and reproducibility/claim boundaries. Supplementary covers heuristic search success, three strictly separated restricted/binary task strata, and supplementary reproducibility/claim boundaries.

After one import regression was fixed, focused tests passed 48. Staged review then closed duplicate-receipt and row/source-hash tampering and added a negative test. Final focus passes 30; complete pytest passes 739 with two skips/11 subtests; unittest passes 177 with one skip; compileall passes. Config/source/metric-registry hashes are `1bcd1386...`, `fba949d2...`, and `98ae57b6...`. Only synthetic temporary fixtures exercised the table writer; no real scientific value, table, figure, promotion, release or manuscript edit exists.
