...
The first two clean core attempts are failed forensic outputs, not partial release candidates. The second reached the benchmark but stopped before policy fitting on a now-repaired registry/report-subset integration defect. A third new-run core and supplementary build, strict read-only validation and pointer promotion remain technical blockers. Benchmark artifacts from the failed run may not be reused. Ethics review, dataset source/licence/citation verification and historical repository sanitation remain independent manual blockers.
