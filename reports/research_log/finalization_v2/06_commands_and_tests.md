...
Complete pytest passed 703 with two skips/11 subtests in 120.78 seconds (124.708 wall). Unittest passed 176/one skip in 7.336 seconds (11.177 wall); compileall passed in 0.114 seconds. No production archive/receipt existed before the clean commit.

Implementation commit `9342b0c` was pushed normally. The production module then exited 0 in 2.24 seconds and wrote the atomic 2,267-byte receipt. Independent receipt validation rebuilt the archive and exited 0 in 1.122 seconds. Exact evidence: 305 allowlisted/closed members, archive size 1,077,955 bytes, uncompressed total 4,005,919 bytes, archive SHA-256 `1917059f...`, member manifest `df89a3a6...`, 14 preserved local files totaling 2,335,429 bytes and receipt SHA-256 `3ddad1b7...`. Zero forbidden tracked/member paths, Git metadata, portability findings, secrets, symlinks, network/API calls or temporary siblings were found; the validation ZIP was removed.

## V2-018 dependency isolation and lock - 2026-07-14

`python -m src.governance.dependency_contract --validate-only` passes with 22 direct packages and 96 exact lock pins. The corrected focused command over dependency, input-snapshot, Unit 2G backward-identity, sanitized-export, INX-provenance and builder-path contracts passed 79 in 4.69 seconds (7.758 wall). Core and development `pip install --dry-run --ignore-installed` commands exited 0 in 5.728 and 20.953 seconds respectively.

The complete pytest wrapper's first five-second tool timeout stopped before a test result and left no Python/pytest process. The properly bounded unchanged command passed 711/2 skips/11 subtests in 122.21 seconds (126.185 wall). Unittest passed 176/one skip in 7.361 seconds (11.192 wall); compileall passed in 0.114 seconds.

A new OS-temp CPython 3.14 venv installed `requirements-core.txt`. The original wrapper timed out while its exact pip process remained live; PID/ancestry/output-directory inspection proved its run identity, so the process was allowed to finish without duplicate or termination. Subsequent `pip check`, 13 direct imports and production environment validation passed with 31 distributions and inventory SHA-256 `a7ac622b...`. The exact temp path is deliberately omitted from persistent publication documentation.

Final review exposed a Linux-development portability defect: the validator required marker-declared `pywin32` even where pip skips it. Production now permits only that exact Windows marker and evaluates it before the environment direct-set check. Final focus passed 80 in 5.07 seconds (8.245 wall); the post-fix full suite passed 712/2 skips/11 subtests in 122.06 seconds (126.148 wall); unittest passed 176/one skip in 7.303 seconds (11.100 wall); compileall passed in 0.122 seconds.

Checkpoint `498e8ad` was pushed. Clean receipt generation exited 0 in 0.685 seconds; independent static/hash/ancestor validation exited 0 in 0.218 seconds. Receipt facts: source tree `a2b361b8...`, config `65519abf...`, constraints `482cbf32...`, 96 pins, 22 declared direct packages, 31 installed distributions, 13 core direct versions, inventory `a7ac622b...`, zero missing/unlocked/mismatched/core-forbidden and receipt SHA-256 `8b77e727...`. The exact temporary environment was removed after a contained-path/no-owner check; a first check self-matched and failed safely before removal.

## V2-020 complete-build no-network/API boundary - 2026-07-14
...
`python -m tools.canonical_manuscript_asset_export --validate-only` returned `status=passed`: 109 files/10,338,351 bytes, 7 main PNG, 7 main SVG, 3 supplementary PNG, 3 supplementary SVG, 8 main tables, manifest `fbe7355b...`. The separate local-source parity call returned `status=passed`, `asset_files=108`, and `source_bindings_checked=308` after the same call revalidated all canonical closed-world bytes.

Focused command over the new export, final-evidence manifest/hash, and sanitized-publication contracts passed 17 tests with one skip in 4.15 seconds (7.357 wall) after correcting two test-only field-name assumptions reported by the first run. The single full regression passed 758/two skips/11 subtests in 129.91 seconds (133.988 wall). Unittest passed 179/one skip in 8.214 seconds (12.246 wall); compileall over `src`, `tests`, and `tools` passed in 0.312 seconds.

The final candidate scan passed: 109 files/10,338,351 bytes, maximum file 2,195,799 bytes, 43 valid README local links, no >=10 MiB figure, prohibited extension, staging/temp path, raw/model/row-level artifact, secret, machine path, active `leakage-safe` term, forbidden primary SHAP feature, manuscript change, or scientific-source/config change. Representative `git check-ignore -v --no-index` probes confirmed the complete canonical root, raw data, model/OOF/local-SHAP paths remain ignored and the compact asset README is not ignored.

The exact staged allowlist passed with 137 Git change records, 109 compact assets, zero unstaged/untracked/unexpected/prohibited paths, zero tracked canonical-root files and all 545 canonical files preserved locally. All 109 staged asset blobs matched the manifest hashes after byte-preserving Git attributes were applied. Commit `902d4041fce94139ba58cb7c982f9a62b8427611` passed the clean repository gate and package validator, then normal push advanced the required remote branch from `7bb2317` to `902d404` in 7.229 seconds. GitHub authentication succeeded.
