...
V2-018 separates the canonical core, approved supplementary, explicitly legacy-optional and development graphs. Supplementary adds no third-party package beyond core; the compatibility entry point resolves core only; development includes the legacy group solely for full historical test collection. One sorted exact CPython 3.14 constraints file covers all declared direct and transitive distributions. The production validator rejects unsafe directives, unbounded or misplaced direct requirements, missing/duplicate/unsorted pins, forbidden core packages, Python drift, unlocked installed packages and broken `pip check`. Run-input snapshots and source identity bind every dependency file. A fresh isolated core environment and exact clean-commit receipt pass. RC-19 is engineering-resolved; cross-platform hosted execution remains the separate V2-019 CI issue.

## RC-20 - Offline Claims Were Stage-Local Rather Than Complete-Build Enforcement

Several stage receipts recorded zero network/API calls and the benchmark trial patched a subset of socket operations, but the public complete builder had no runtime boundary. Canonical loading retained a configured download implementation, environment credentials could remain visible, caught failures were not globally fatal, and a child process could bypass parent socket patches. Zero-valued metadata alone could therefore overstate enforcement.

V2-020 wraps the entire public scientific build, from preflight through final validation, in one process-wide boundary. It blocks DNS, TCP, UDP, listener and send paths across threads; clears/restores common paid-API credentials; records prohibited attempts and refuses completion even if caught; rejects shell/non-Git child commands; and allows only exact local read-only Git provenance subcommands. Complete package status must equal the zero-attempt policy. Dynamic tests exercise each route and the real config still fails closed at its release gate. RC-20 is locally implementation-resolved; V2-019 must run the assertion in hosted CI and the final real package must emit the policy.
