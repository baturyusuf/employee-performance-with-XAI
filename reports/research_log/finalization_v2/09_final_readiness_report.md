...
**Not ready.** Actual-input/side-input binding, core/supplementary isolation, shared-fold benchmarking, exact-model OOF SHAP, matched policy ablation, cross-fitted sigmoid calibration, support-aware subgroup/proxy contracts, conservative HRDataset replication, closed-world lock-aware completion/promotion, fail-closed seven-figure package admission and the supplementary heuristic-search contract are implemented and tested. Unit 2G passes reusable scientific validation; V2-029 pins reporting semantics; V2-032 rejects conflicting/historical figure packages; V2-012 freezes exact OOF/training-only search, identities, scopes, budgets, denominators, uncertainty and nonprescriptive terminology. The current full gate passes 687 pytest tests with 2 skips and 11 subtests plus 176 unittest tests with 1 skip. Production figures/tables, V2-013 related-task external implementation, dependency locking, CI/release workflows and final all-stage integration remain open.

## Scientific readiness

**Not ready.** The verified noncanonical four-model trial did not trigger the baseline superiority gate, so XGBoost remains the predeclared XAI reference. The full HRDataset policy-A stage completed with 10 outer x 5 inner tuning, exact-model OOF SHAP and 5,000-draw uncertainty. Raw conservative macro-F1 is `0.666355`; sigmoid improves log loss/Brier but lowers macro-F1 and predicts no class-4 argmax cases; the temporality-restricted audit shows a large performance loss. These are stage-validation findings, not frozen manuscript claims. Department proxy estimation is unsupported because singleton-class training support is insufficient. V2-012 produced no retained scientific result; its bounded diagnostic proves execution/schema behavior only. No same-config complete canonical core or supplementary package exists.
...
**Blocked.** Pinned user-provided/acquisition contracts pass for current local inputs, but raw-data distribution and source/licence authenticity still require the accepted sanitized-publication workflow and manual verification. The 65.4 MB noncanonical Unit 2G package is preserved locally and removed from the branch tip by forward cleanup `b7b2ad3` under D5. The small scientific checkpoint is pushed, but the large package remains in history because rewriting/force-push is prohibited. No ambiguous licence/source judgement has been invented.
...
See `02_issue_register.csv` and `08_manual_submission_blockers.md`. D1-D5, cross-fitted sigmoid, HRDataset policy A and the Option-A replacement figure plan are accepted. The real HRDataset stage and reusable artifact audit are complete; V2-029, V2-032 and V2-012 are implementation-complete. History sanitation remains unauthorized and open. The exact next unit is V2-013 task-bounded supplementary external evidence, followed by the task/metric schema, generator implementation, dependency/CI contracts, sanitized publication transition and final integration. Ethics and source/licence authenticity remain external manual gates; claim-matrix author approval occurs only after the final package exists.
