...
The completed Phase 2A [`SHAP aggregation, stability, and faithfulness protocol`](reports/research_log/major_revision_v3/SHAP_STABILITY_FAITHFULNESS_PROTOCOL.md) and governed [`compact evidence package`](reports/research_log/major_revision_v3/phase2a_shap_stability_faithfulness/README.md) preserve exact outer-model TreeSHAP, document raw-margin multiclass grouping, and add separately labelled seed stability, 80% outer-training resampling stability, and training-fold median/mode deletion against 20 random baselines. Mean top-5 Jaccard is 1.0000 in all three stability comparisons; all-feature Spearman is lower across canonical folds (0.9066) than model seeds (0.9945) or resamples (0.9847). SHAP-guided deletion produces larger descriptive probability drops than random deletion at top 1/3/5, but masking may create out-of-distribution hybrids and provides no causal or human-usefulness evidence.

The Phase 2B [`extended calibration diagnostics protocol`](reports/research_log/major_revision_v3/CALIBRATION_DIAGNOSTICS_PROTOCOL.md) and governed [`compact evidence package`](reports/research_log/major_revision_v3/phase2b_calibration_diagnostics/README.md) preserve the exact predeclared cross-fitted one-vs-rest sigmoid method and add explicit top-label, classwise, and ordinal-cumulative reliability/ECE definitions, normalized RPS/cumulative Brier, and descriptive pooled-OOF calibration intercept/slope. The clean-commit run is independently recomputed: sigmoid improves log loss, multiclass Brier, macro classwise ECE, cumulative ECE, and RPS point estimates but worsens top-label ECE from 0.0375 to 0.0419. No new performance model or probability calibrator is fitted and no method is selected from test results.

The Phase 2C [`subgroup and proxy-use protocol`](reports/research_log/major_revision_v3/SUBGROUP_PROXY_USE_PROTOCOL.md) and governed [`compact evidence package`](reports/research_log/major_revision_v3/phase2c_subgroup_proxy_use/README.md) provide complete subgroup metric and n=20/30/50 sensitivity grids, multiplicity-aware exploratory P3 gap intervals, exact P3-versus-P3-minus-JobRole prediction changes, and marginal plus department-conditional JobRole perturbations. The P3-minus-JobRole comparison changes 10.75% of argmax predictions and lowers macro-F1 by 0.0330; marginal perturbations are larger than department-conditional perturbations. Department reconstructability remains explicitly separate from performance-model dependence, and no fairness, discrimination, causal, or deployment claim is permitted.

The Phase 3A [`HRDataset_v14 target-mapping and repeated-CV sensitivity protocol`](reports/research_log/major_revision_v3/HRDATASET_SENSITIVITY_PROTOCOL.md) retains the primary mapped 2/3/4 target and adds a semantically ordered four-class sensitivity preserving PIP and Needs Improvement separately. Exact run `phase3a_v3_20260907T141213Z_f6e6a0a` completed five prespecified 5×5 nested-CV repetitions with training-only sigmoid calibration and three naive baselines; its 18-file local evidence passed independent fold, selection, baseline, calibration, and result recomputation with zero sigmoid replay error. The governed [compact aggregate evidence](reports/research_log/major_revision_v3/phase3a_hrdataset_sensitivity/README.md) contains 14 files/409,991 bytes with manifest SHA-256 `415f1bc5d346c282e51dcc1212335c80c0ee3cd7a673180b22bc4b36f52d1651`. This is independent protocol replication, not external validation or locked-model transport.

The completed Phase 3B [`core data-quality report`](reports/research_log/major_revision_v3/phase3b_data_quality/DATA_QUALITY_REPORT.md) audits both canonical core datasets without changing source rows. INX has no missing cells, duplicate rows, near-constant columns, identifier leakage into P3, or violations of the declared domain/tenure rules. HRDataset_v14 has 215 missing cells and three rules with two findings each: review-before-hire timing, performance text/code disagreement, and minority department text within a department-code group. The governed 13-file/53,868-byte package has manifest SHA-256 `07083ec83ac01e9393995b30c697190e2ee5221cd54aa3b54fbde01072f57eea`; it contains schemas and aggregate profiles only, not raw or row-level values. Recorded ratings are not established as true capability, objective productivity, or future potential, and the audit creates no external-generalization claim.

The completed Phase 4A [`source-verified literature and novelty package`](reports/research_log/major_revision_v3/phase4a_literature/README.md) freezes a bounded 25-work positioning set: four exact-INX studies, one exact-HRDataset_v14 study, four other close HR empirical studies, and sixteen methodological/governance sources. Sixteen DOIs resolve to exact Crossref records, eight works have no assigned DOI in the verified primary record, and one publisher-asserted DOI remains explicitly unresolved. The nine-file/58,534-byte package includes the search protocol, complete evidence ledger, capability table, verified bibliography, exclusions, bounded novelty statement, provenance receipt, and closed-world manifest. It does not claim an exhaustive review or world-first contribution and does not modify the manuscript bibliography.

The Phase 4B [`provenance, ethics, and immutable-release readiness package`](reports/research_log/major_revision_v3/phase4b_release_readiness/README.md) separates local byte identity, upstream authenticity, licence evidence, redistribution permission, citation authority, and ethics status for all four physical datasets. The validated 12-file/38,324-byte package inventories ten compact evidence components (203 files/13,056,035 bytes), eight declaration fields, and eleven open blockers. Its release state is deliberately `blocked_preparation_only`: no raw dataset is approved for redistribution; the repository has no detected software licence; five raw-data paths remain in Git history; and no final commit, remote tag, GitHub release, archive record, DOI, or ethics determination is claimed. The proposed future tag is `v1.0.0-paper-revision`, but it must not be created or published without the remaining approvals and explicit authorization.
...
The additive [`Round 2 major-revision plan`](reports/research_log/major_revision_round2/ROUND2_PLAN.md) freezes the selection-objective sensitivity, training-only empirical-prior probability baseline, limited HR target-alias sensitivity, publication-table extractions, method/literature expansion, new claim-matrix gate, and manuscript/reviewer-response workflow. Baseline commit `0805b793a3b2a2adb077dfdf7f271e309bd5774f` and all Phase 1–5 evidence remain historical and immutable. The plan has now been executed under the separately approved Round 2 claim boundary.

The independently validated Round 2 [`selection-objective sensitivity package`](reports/research_log/major_revision_round2/selection_objective_sensitivity/README.md) separates leader changes, full-ordering changes, candidate changes, and score magnitudes across macro-F1- and QWK-selected regimes. Companion reports document the [`selection results`](reports/research_log/major_revision_round2/SELECTION_OBJECTIVE_SENSITIVITY.md), [`training-only empirical-prior probability baseline`](reports/research_log/major_revision_round2/PROBABILITY_BASELINE_REPORT.md), and [`extreme-class behavior`](reports/research_log/major_revision_round2/PER_CLASS_EXTREME_CLASS_REPORT.md). Row-level OOF evidence remains local and unpublished.
...
The additive Round 2 [`core-method literature package`](reports/research_log/major_revision_round2/LITERATURE_V4/README.md) source-verifies proportional-odds, ordinal classification/evaluation, QWK, RPS/proper scoring, Random Forest, XGBoost, and LightGBM references. Following digest-specific approval, its nine BibTeX entries were added to the manuscript bibliography without altering the historical Phase 4A package.

The approved Round 2 [`claim matrix`](reports/research_log/major_revision_round2/round2_claim_matrix/README.md) contains 129 classified claims, including 117 active rewrite claims and 99 exact numerical claims, under SHA-256 `751208d036597461606bd02d68bfa9e642a4aa5ecf5df2df3fd7e46b99084aea`. The [`Round 2 manuscript package`](reports/research_log/major_revision_round2/round2_manuscript/README.md) applies that sole scientific boundary to the Markdown/LaTeX manuscript, 34-work bibliography, reviewer response, and final review simulation. It retains 7 figures, expands the main text to 11 tables, removes visible claim codes from ordinary prose, and stores complete source identities in the [`Supplementary Evidence Ledger`](reports/research_log/major_revision_round2/SUPPLEMENTARY_EVIDENCE_LEDGER_ROUND2.csv). The status is scientifically revised but not submission-ready or release-authorized; ethics/IRB, author declarations, rights, licence, history, tag, release, and DOI blockers remain open.
