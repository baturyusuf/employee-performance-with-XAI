...
The Phase 2C [`subgroup and proxy-use protocol`](reports/research_log/major_revision_v3/SUBGROUP_PROXY_USE_PROTOCOL.md) freezes complete subgroup metric and n=20/30/50 sensitivity grids, multiplicity-aware exploratory P3 gap intervals, exact P3-versus-P3-minus-JobRole prediction changes, and marginal plus department-conditional JobRole perturbations. Department reconstructability is explicitly kept separate from performance-model dependence. The fit-free exact run and independent full recomputation pass; only the governed compact aggregate package remains to be published.
