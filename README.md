...
- Recovery on 2026-07-14 found clean HEAD `eab2b32` exactly synchronized with `origin/finalization/leakage-aware-v2`. The earlier HTTPS authentication blocker is therefore resolved. The full evidence blobs remain in public history, which is still a publication-hygiene blocker under the no-history-rewrite rule.
