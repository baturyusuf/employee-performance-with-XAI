...
- Recovery on 2026-07-14 found clean HEAD `eab2b32` exactly synchronized with `origin/finalization/leakage-aware-v2`, so the earlier backlog had reached the remote. The new Unit 2G checkpoint commit is local `0e3f50c`; its final normal push attempts timed out once at Credential Manager and then failed noninteractively because no HTTPS username was available. The remote remains at `eab2b32`. The full evidence blobs remain in public history, which is still a publication-hygiene blocker under the no-history-rewrite rule.
