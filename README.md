...
- Migration-commit run `canonical_v2_20260714T192934Z_0c2868b` failed after calibration computation when a transient Windows file handle blocked the atomic directory rename; cleanup then masked the first exception. It is preserved as failed and inadmissible. Every atomic directory publisher now uses a bounded retry only for Windows access/sharing/lock errors, never overwrites an existing destination, and retains the primary exception if cleanup also fails. The final focused and complete suites pass; a fresh exact-commit run is required.
- V2-020 now wraps the complete core/supplementary scientific build in a process-wide no-network/no-paid-API boundary. DNS, TCP, UDP, listener and socket-send operations fail; caught attempts poison completion; API credentials are cleared/restored; shell, non-Git and remote-Git subprocesses are rejected; package status must carry the exact zero-attempt policy. Hosted enforcement passed in V2-019/V2-021 CI, including run `29353799148`; the current complete local suite passes 752 tests, 2 skips and 11 subtests.
