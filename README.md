...
The Phase 3A [`HRDataset_v14 target-mapping and repeated-CV sensitivity protocol`](reports/research_log/major_revision_v3/HRDATASET_SENSITIVITY_PROTOCOL.md) retains the primary mapped 2/3/4 target and adds a semantically ordered four-class sensitivity preserving PIP and Needs Improvement separately. Both are evaluated through five prespecified 5×5 nested-CV repetitions with training-only sigmoid calibration and majority, stratified, and ordinal-median baselines. This is independent protocol replication, not external validation or locked-model transport; result publication remains pending exact clean-commit execution and independent validation.
