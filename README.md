...
The current branch-tip change removes 14 declared source, interim, row-level processed, and fitted-working-data paths from Git tracking while preserving and ignoring all 2,335,429 local bytes. Dataset cards, schema mappings, documentation, and placeholders remain tracked. The history-free export validator is implemented but its clean-commit receipt is pending this checkpoint. Earlier Git history still contains raw files and is not rewritten; source/licence verification and any public-history strategy remain manual blockers.
