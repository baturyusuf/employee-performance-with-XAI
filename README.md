...
- Before V2-021 implementation, clean HEAD `ea34ed6` was exactly synchronized with `origin/finalization/leakage-aware-v2`. Unit 2G, V2-032, dependency, offline-runtime, and CI checkpoints were sent by normal authenticated pushes. Hosted CI run `29352015443` passed at that exact receipt commit. The full noncanonical evidence blobs remain in public history, which is still a publication-hygiene blocker under the no-history-rewrite rule.
...
- These downstream stages are implemented but have **not** yet been regenerated together as one canonical scientific package. Final benchmark, policy, calibration, SHAP, subgroup/proxy, and external numbers still require one clean current-commit run. The replacement seven-figure plan, production generator, fail-closed package validator, controlled Python dependency split/lock, and hosted CI are implemented. Real current-run figure artifacts, core tables, and the complete two-scope rebuild remain unfinished.
...
- V2-020 now wraps the complete core/supplementary scientific build in a process-wide no-network/no-paid-API boundary. DNS, TCP, UDP, listener and socket-send operations fail; caught attempts poison completion; API credentials are cleared/restored; shell, non-Git and remote-Git subprocesses are rejected; package status must carry the exact zero-attempt policy. Hosted enforcement passed in V2-019 CI, including receipt run `29352015443`; the complete local suite now passes 733 with 2 skips/11 subtests.
