...
The Phase 2B [`extended calibration diagnostics protocol`](reports/research_log/major_revision_v3/CALIBRATION_DIAGNOSTICS_PROTOCOL.md) now has a validated, hash-bound implementation. It preserves the exact predeclared cross-fitted one-vs-rest sigmoid method and adds explicit top-label, classwise, and ordinal-cumulative reliability/ECE definitions, normalized RPS/cumulative Brier, and descriptive pooled-OOF calibration intercept/slope. No new model or probability calibrator is fitted, no method is selected from test results, and mixed directions—including the worse sigmoid top-label ECE point estimate—must remain visible. Clean-commit execution and independently validated compact results are pending.
